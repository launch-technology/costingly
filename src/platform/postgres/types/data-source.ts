...
export interface DataSource extends Executor {
  ...
  describe(): string;
  /**
   * Release whatever this holds between calls.
   *
   * Optional because not every implementation holds anything: one that opens a
   * connection per call has nothing to release, and should not have to carry an
